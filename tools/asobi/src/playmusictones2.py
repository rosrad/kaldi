...
def play_music(seq, base=4, beatlen=1):
    beats = 0
    nowtime = time.time()
    for i in seq:
    ...
    sleeptime = nowtime + playtime - time.time()
    print 'beats: %s, playtime: %s, sleeptime: %s' % \
        (beats, playtime, sleeptime)
...
play_music(score, base=4, beatlen=0.5)
device.close()
